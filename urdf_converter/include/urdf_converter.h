...
moveit_msgs::CollisionObject makeCollObjMsg(urdf::LinkConstSharedPtr link)
{
  ...
  co.named_frames.resize(link->child_joints.size());
  for (std::vector<urdf::JointSharedPtr>::const_iterator child = link->child_joints.begin(); child != link->child_joints.end(); child++)
  {
    ...
    co.named_frames[count].position.x = (*child)->parent_to_joint_origin_transform.position.x;
    co.named_frames[count].position.y = (*child)->parent_to_joint_origin_transform.position.x;
    co.named_frames[count].position.z = (*child)->parent_to_joint_origin_transform.position.x;
    co.named_frames[count].orientation.x = (*child)->parent_to_joint_origin_transform.rotation.x;
    co.named_frames[count].orientation.y = (*child)->parent_to_joint_origin_transform.rotation.y;
    co.named_frames[count].orientation.z = (*child)->parent_to_joint_origin_transform.rotation.z;
    co.named_frames[count].orientation.w = (*child)->parent_to_joint_origin_transform.rotation.w;
    count++;
  }
}
