...
from aist_robotiq.epick_urcap import RobotiqEPickURCap
...
def mainLoop(ur_address):
    ...
    gripper = RobotiqEPickURCap(ur_address)

    # The Gripper status
    pub = rospy.Publisher('/status', amsg.EPickStatus, queue_size=3)
    # The Gripper command
    rospy.Subscriber('/command', amsg.EPickCommand, gripper.sendCommand)
...
if __name__ == '__main__':
    rospy.init_node('epick_urcap_driver')
    try:
        mainLoop(sys.argv[1])
