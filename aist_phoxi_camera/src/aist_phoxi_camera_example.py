...
from aist_phoxi_camera.srv import *
from std_srvs.srv import *

if __name__ == "__main__":
    rospy.init_node('aist_phoxi_camera_example', anonymous=True)

    rospy.wait_for_service('aist_phoxi_camera/get_device_list')
    ...
    try:
        # get device list
        res_get_device_list = rospy.ServiceProxy('aist_phoxi_camera/get_device_list', GetStringList)()
        rospy.loginfo("devices" + str(res_get_device_list.out))

        # start data acquisition
        res_star_acq = rospy.ServiceProxy('aist_phoxi_camera/start_acquisition', Trigger)()

        # capturing frames
        while not rospy.is_shutdown():
            res_trigger = rospy.ServiceProxy('aist_phoxi_camera/trigger_frame', Trigger)()
            if (res_trigger.success == False):
                rospy.logerr("Trigger frame request to the phoxi camera was failed.")

            res_get_frame = rospy.ServiceProxy('aist_phoxi_camera/get_frame', GetFrame)(0, False)
